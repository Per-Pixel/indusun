...
export async function GET(
  ...
  try {
    ...
    const name  = (data as any)["broker's_name"] || '';
...
export async function PUT() {
  return NextResponse.json(
    { error: 'Editing brokers is not supported on the read-only master data table.' },
    { status: 405 }
  );
}
...
export async function DELETE() {
  return NextResponse.json(
    { error: 'Deleting brokers is not supported on the read-only master data table.' },
    { status: 405 }
  );
}
